...
def index(request):
    """俳句一覧"""
    haiku = Register.objects.all()
    data_dictionary = {'Register': haiku}
    return render(request, 'register/index.html', data_dictionary)

def create(request):
    """俳句追加画面"""
    if request.method == 'POST':
        haiku = Register.objects.create(
            kamigo = request.POST['kamigo'],
            nakasiti = request.POST['nakasiti'],
            simogo = request.POST['simogo'],
            haijin = request.POST['haijin'],
            haiku_id = request.POST['haiku_id'],
        )
        haiku.save()
        return redirect("register:index")
    return render(request, 'register/create.html')

def delete(request, haiku_id):
    haiku = Register.objects.get(pk=haiku_id)
    haiku.delete()
